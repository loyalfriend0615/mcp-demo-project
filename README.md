...
> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install mcp-demo-project`
...
1. Via the `MCP_DEMO_PROJECT_LOG` environment variable
...
const client = new McpDemoProject({
  logger: logger.child({ name: 'McpDemoProject' }),
...
const client = new McpDemoProject({ fetch });
```
...
```ts
import McpDemoProject from 'npm:mcp-demo-project';
