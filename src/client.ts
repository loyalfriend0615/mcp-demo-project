...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['MCP_DEMO_PROJECT_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['MCP_DEMO_PROJECT_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Mcp Demo Project API.
 */
export class McpDemoProject {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['MCP_DEMO_PROJECT_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('MCP_DEMO_PROJECT_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.McpDemoProjectError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the McpDemoProject client with an apiKey option, like new McpDemoProject({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? McpDemoProject.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('MCP_DEMO_PROJECT_LOG'), "process.env['MCP_DEMO_PROJECT_LOG']", this) ??
      defaultLogLevel;
  ...
  static McpDemoProject = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static McpDemoProjectError = Errors.McpDemoProjectError;
